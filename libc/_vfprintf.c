...
extern int fputs_unlocked(register const char* __restrict s, FILE* __restrict stream);
...
#define _outnstr(stream, string, len)	((len > 0) ? fwrite((const unsigned char *)(string), 1, len, stream) : 0)
...
#endif
#endif
			uintmax_t _t = _load_inttype(ppfs->conv_num == CONV_p ? PA_FLAG_LONG : *argtype & __PA_INTMASK,
											*argptr, base);
			
			s = _uintmaxtostr(buf + sizeof(buf) - 1, _t, base, alphacase);
